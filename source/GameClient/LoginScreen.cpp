...
void LoginScreen::Build(void) {
	...
	m_textLoading.Load("Loading... Please wait", { 20,150,0 }, m_app->window, m_app->renderer, m_app->font);
...
void LoginScreen::Update(void) {
	...
	if (m_app->nick.empty()) {
		...
		if (m_app->inputManager.isKeyPressed(SDLK_RETURN) && !m_inputText.empty()) m_app->nick = m_inputText;
